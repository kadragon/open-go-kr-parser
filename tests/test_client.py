...
class TestOpenGoKrClient:
    ...
    def _make_html_response(self, rtn_list: list[dict[str, str]], rtn_total: int) -> str:
        ...
        result = {"rtnList": rtn_list, "rtnTotal": rtn_total}
        return f"<html><script>var result = {json.dumps(result)};</script></html>"
    ...
    @responses.activate
    def test_fetch_documents_missing_result_raises_error(
        self, client: OpenGoKrClient
    ) -> None:
        """Raise error when result data not found in HTML."""
        responses.add(
            responses.POST,
            self.PAGE_URL,
            body="<html><body>No result here</body></html>",
            status=200,
        )
